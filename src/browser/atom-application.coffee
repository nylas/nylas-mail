...
throw new Error("This file is deprecated. Use edgehill-application instead.")
...
class AtomApplication
  ...
  openWithOptions: ({pathsToOpen, urlsToOpen, test, pidToKillWhenClosed, devMode, safeMode, newWindow, specDirectory, specFilePattern, logFile}) ->
    if test
  ...
  handleEvents: ->
    ...
    ipc.on 'action-bridge-message', (event, args...) =>
      win = BrowserWindow.fromWebContents(event.sender)
      @windows.forEach (atomWindow) ->
        return if atomWindow.browserWindow == win
