...
function stripTrackingLinksFromBody(originalBody) {
  ...
  //
  // See https://regex101.com/r/Tr0LLT/1 for urlLinkTagRegex on example
  // link.
  // The link-tracking/lib/link-tracking-composer-extension.es6
  // will add a `redirect` query param that has the original url.
  body = body.replace(urlLinkTagRegex(), (match, prefix, href, suffix, content, closingTag) => {
    if (!/nylas\.com/.test(href)) return match
    const originalUrl = (url.parse(href).query || {}).redirect
...
function addRecipientToUrl(originalUrl, email) {
  const parsed = url.parse(originalUrl)
  parsed.query.recipient = email
 ...
 * link.
 */
function addRecipientToTrackingLinks({baseMessage, recipient, usesOpenTracking, usesLinkTracking} = {}) {
