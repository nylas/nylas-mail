...
export default class MessageParticipants extends React.Component {
  ...
  _renderExpanded() {
    ...
    if (this.props.to.length > 0) {
      expanded.push(
        this._renderExpandedField('to', this.props.to)
      );
    }

    if (this.props.cc.length > 0) {
      expanded.push(
        this._renderExpandedField('cc', this.props.cc)
      );
    }

    if (this.props.bcc.length > 0) {
      expanded.push(
        this._renderExpandedField('bcc', this.props.bcc)
      );
    }
  ...
  _renderCollapsed() {
    ...
    if (this.props.from.length > 0) {
      childSpans.push(
        <span className="participant-name from-contact" key="from">{this._shortNames(this.props.from)}</span>,
      );
    }
    ...
    if (toParticipants.length > 0) {
      childSpans.push(
        ...
        <span className="participant-name to-contact" key="to-value">{this._shortNames(toParticipants)}</span>,
      );
    }
